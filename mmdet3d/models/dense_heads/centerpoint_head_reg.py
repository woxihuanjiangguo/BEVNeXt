...
class CenterHeadReg(BaseModule):
    ...
    def __init__(self,
        ...
        super(CenterHeadReg, self).__init__(init_cfg=init_cfg)
    ...
    def forward_single(self, x, heatmap):
        ...
        for idx, task in enumerate(self.task_heads):
            ret_dict = task(x)
            ret_dict.update(heatmap[idx][0])
            ret_dicts.append(ret_dict)

        return ret_dicts

    def forward(self, feats_heatmap):
        """Forward pass.
        ...
        """
        feats, heatmap = feats_heatmap
        return multi_apply(self.forward_single, feats, heatmap)
