...
@PIPELINES.register_module()
class PointToMultiViewDepthFgBg(object):

    def __init__(self, grid_config, downsample=1):
        self.downsample = downsample
        self.grid_config = grid_config

    def points_img(self, pts, lidar2img, post_rots, post_trans, cid):
        points_img = pts.tensor[:, :3].matmul(
            lidar2img[:3, :3].T) + lidar2img[:3, 3].unsqueeze(0)
        points_img = torch.cat(
            [points_img[:, :2] / points_img[:, 2:3], points_img[:, 2:3]],
            1)
    ...
    def __call__(self, results):
        imgs, rots, trans, intrins = results['img_inputs'][:4]
        post_rots, post_trans, bda = results['img_inputs'][4:]
        depth_map_list, depth_fg_list, depth_bg_list = [], [], []
        for cid in range(len(results['cam_names'])):
            cam_name = results['cam_names'][cid]
            lidar2lidarego = np.eye(4, dtype=np.float32)
            lidar2lidarego[:3, :3] = Quaternion(
            ...
            pts_img = self.points_img(results['points'], lidar2img, post_rots, post_trans, cid)
            pts_img_fg = self.points_img(results['points_fg'], lidar2img, post_rots, post_trans, cid)
            pts_img_bg = self.points_img(results['points_bg'], lidar2img, post_rots, post_trans, cid)

            depth_map = self.points2depthmap(pts_img, imgs.shape[2],
                                             imgs.shape[3])
            depth_map_fg = self.points2depthmap(pts_img_fg, imgs.shape[2],
                                             imgs.shape[3])
            depth_map_bg = self.points2depthmap(pts_img_bg, imgs.shape[2],
                                             imgs.shape[3])

            depth_map_list.append(depth_map)
            depth_fg_list.append(depth_map_fg)
            depth_bg_list.append(depth_map_bg)

        results['gt_depth'] = torch.stack(depth_map_list)
        results['gt_depth_fg'] = torch.stack(depth_fg_list)
        results['gt_depth_bg'] = torch.stack(depth_bg_list)
        return results




def mmlabNormalize(img, vov_style=False):
    if not vov_style:
        # default setting
...
class PrepareImageInputs(object):
    ...
    def get_inputs(self, results, flip=None, scale=None):
        ...
        for cam_name in cam_names:
            ...
            canvas.append(torch.tensor(np.array(img)))
            imgs.append(self.normalize_img(img, self.vov_style))

            if self.sequential:
                assert 'adjacent' in results
                adj_stuff = results['adjacent']
                if self.future:
                    adj_stuff = adj_stuff + results['adjacent_future']
                for adj_info in adj_stuff:
                    ...
                    img_adjacent = self.img_transform_core(
                        ...
                        flip=flip,
                        rotate=rotate)
                    imgs.append(self.normalize_img(img_adjacent, self.vov_style))
    ...
    def get_adjacent_bboxes(self, results):
        ...
        for adj_info in results['adjacent_future']:
    ...
    def align_adj_bbox2keyego(self, results):
        ...
        ret_list = []
        for idx, adj_info in enumerate(results['adjacent_future']):
